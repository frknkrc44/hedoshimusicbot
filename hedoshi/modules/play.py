    ...
    if message.reply_to_message:
        ...
        if len(message.command) > 1:
            ...
            if parse_telegram_url(command)[0]:  # type: ignore
                ...
                path, params = youtube.download_media(command, not video_mode)
                if path:
                    await start_stream(msg, path, video_mode, params)
                    return
            else:
                if search := yt_search.search_query(command):
                    if youtube.is_valid(search):  # type: ignore
                        path, params = youtube.download_media(search, not video_mode)  # type: ignore
                        if path:
                            await start_stream(msg, path, video_mode, params)
                            return
